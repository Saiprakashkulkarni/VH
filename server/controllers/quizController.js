...
const getQuiz = asyncHandler(async (req, res) => {
  ...
  } catch { /* fall through to static quiz */ }
...
const submitQuiz = asyncHandler(async (req, res) => {
  ...
  if (user && score >= QUIZ_BONUS_THRESHOLD) {
    user.readinessScore = Math.min(MAX_READINESS_SCORE, user.readinessScore + SCORE_QUIZ_BONUS);
    await user.save();
  }

  res.json({
    success: true,
    data: { score, total: quiz.questions.length, percentage: Math.round((score / quiz.questions.length) * 100), results, readinessBonus: score >= QUIZ_BONUS_THRESHOLD },
  });
});
