...
import {
  FiGlobe, FiCopy, FiCheck, FiVolume2, FiSend, FiRotateCcw, FiArrowDown, FiRepeat,
} from 'react-icons/fi';
...
export default function TranslatorPage() {
  ...
  useEffect(() => {
    ...
    // Trigger voice loading so they are available when the user first speaks
  ...
  const handleSpeak = (text, langCode) => {
    ...
    // Find best voice with multiple fallback strategies
    const bestVoice = (
      // 1. Exact match: "hi-IN" === "hi-IN"
      allVoices.find(v => v.lang === speechCode) ||
      ...
      allVoices.find(v => v.lang.startsWith(langCode)) ||
      // 4. Google voice for the language (highest quality)
      allVoices.find(v => v.name.toLowerCase().includes('google') && v.lang.startsWith(langCode)) ||
      // 5. Any voice mentioning the language name
      (langInfo ? allVoices.find(v => v.name.toLowerCase().includes(langInfo.nameEn.toLowerCase())) : null)
    );
