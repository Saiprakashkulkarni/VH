...
/**
 * Create a default voter readiness checklist for a new user.
 * Auto-completes items based on the user's current profile data.
 *
...
const createChecklist = async (user) => {
  ...
  if (user.voterStatus === VOTER_STATUS.REGISTERED) {
...
/**
 * Calculate a voter readiness score (0–100) from profile data.
 *
...
const calcReadinessScore = (data) => {
  let score = 0;
  if (data.voterStatus === VOTER_STATUS.REGISTERED) score += SCORE_REGISTERED;
  else if (data.voterStatus === VOTER_STATUS.APPLIED) score += SCORE_APPLIED;
  if (data.hasVoterId) score += SCORE_HAS_VOTER_ID;
  if (data.age >= VOTING_AGE) score += SCORE_VOTING_AGE;
  if (data.pincode) score += SCORE_HAS_PINCODE;
  return score;
};
...
const completeProfile = asyncHandler(async (req, res) => {
  ...
  user.voterStatus = voterStatus || VOTER_STATUS.UNKNOWN;
  user.hasVoterId = hasVoterId || false;
  user.isFirstTimeVoter = isFirstTimeVoter !== undefined ? isFirstTimeVoter : (age <= FIRST_TIME_VOTER_AGE);
